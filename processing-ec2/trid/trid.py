...
def LoadSmallData(raw_data: bytes):
    datasize = len(raw_data)
    if datasize <= MAX_FILE_SIZE:
        return raw_data.upper()
    else:
        part_size = MAX_FILE_SIZE // 2
        cut_data = raw_data[:part_size]
        cut_data = cut_data + b'|' + raw_data[part_size:]
        return cut_data.upper()
...
def tridAnalyze(raw_data, triddefs, stringcheck=True) -> list[TrIDResult]:
    """
    ...
    for triddef in triddefs:
        ...
        if pts > 0 and stringcheck == True:
            if not filebuffer:
                # filebuffer = LoadDataFromFile(raw_data)
                filebuffer = LoadSmallData(raw_data)
            skipsearch = False
